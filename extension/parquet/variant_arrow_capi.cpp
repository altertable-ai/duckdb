...
string ParquetVariantBytesToJson(ClientContext &context, const_data_ptr_t metadata, idx_t metadata_len,
                                 const_data_ptr_t value, idx_t value_len) {
	Vector metadata_vector(LogicalType::BLOB, 1);
	Vector value_vector(LogicalType::BLOB, 1);
	FlatVector::GetData<string_t>(metadata_vector)[0] = string_t(const_char_ptr_cast(metadata), metadata_len);
...
extern "C" DUCKDB_EXTENSION_API duckdb_state duckdb_parquet_variant_bytes_to_json(duckdb_database database,
	...
	*out_json = nullptr;
	...
	try {
		...
		duckdb::ClientContext context(wrapper->database->instance);
		auto json = duckdb::ParquetVariantBytesToJson(context, metadata, metadata_len, value, value_len);

...
vector<reference<SchemaCatalogEntry>> SystemCatalogScanFunction::GetSchemas(ClientContext &context,
	...
	return Catalog::GetSchemas(context, data.catalog);
}
...
static bool IsDatabaseNameColumn(const LogicalGet &get, const BoundColumnRefExpression &ref) {
	...
	if (ref.binding.column_index < column_ids.size()) {
		...
		if (col_idx.HasPrimaryIndex()) {
			const auto primary = col_idx.GetPrimaryIndex();
			if (primary < get.names.size()) {
				return get.names[primary] == SystemCatalogScanFunction::DATABASE_NAME_COLUMN;
			}
		}
	}
...
void SystemCatalogScanFunction::Serialize(Serializer &serializer, const optional_ptr<FunctionData> bind_data,
                                          const TableFunction &) {
	auto &data = bind_data->Cast<SystemCatalogScanBindData>();
	serializer.WritePropertyWithDefault<string>(100, "catalog", data.catalog);
}
